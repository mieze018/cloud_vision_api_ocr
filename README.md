...
3. PDF の場合、「見開きページを分割する」オプションが表示されます（必要に応じてチェック）
4. 「OCR 開始」ボタンをクリック
5. 処理が開始され、進捗がログに表示されます
6. 完了すると、指定した出力先に Markdown ファイルが生成されます
...
スキャンされた本や雑誌など、**見開き（2ページ分が1つの画像になっている）PDF** を OCR する際に、ページを自動的に左右に分割します。

```
見開きPDFの問題:
┌─────────────────────────────────────────┐
│     ページ7        │     ページ6        │  ← 1つの画像として認識
│   ←←←←←←←←       │    ←←←←←←←←       │
│   縦書き・右から左  │   縦書き・右から左  │
└─────────────────────────────────────────┘
          ↓ Vision API が処理すると...

左右のテキストが混ざって認識される → 読み取り順序がおかしくなる
```

### 使い方

1. PDF ファイルを選択すると、「見開きページを分割する」チェックボックスが表示されます
2. チェックを入れると、読み取り順序を選択できます:
   - **右→左（日本語縦書き）**: 日本の本・雑誌など
   - **左→右（横書き）**: 洋書・横書きの資料など
3. 「OCR 開始」をクリック

### 仕組み

1. 各ページの縦横比を判定（横長 = 見開きと判断）
2. 見開きページを中央で左右に分割
3. 指定された順序（右→左 or 左→右）で新しいページを作成
4. 分割後の PDF を OCR 処理
...
> ⚠️ **実験的機能です**
>
> - 横長のページは全て見開きとして分割されます（単ページの横長画像も分割される）
> - 分割位置は常に中央です（左右のページサイズが異なる場合は調整できません）
> - PDF のみ対応（画像ファイルは非対応）
> - 処理時間が若干増加します
