...
Google Cloud Vision API を使用した大規模 PDF OCR デスクトップアプリケーションです。
数百ページ規模の PDF を非同期バッチ処理で OCR し、Markdown 形式で出力します。
...
- **大容量 PDF 対応**: 数百ページの PDF も非同期バッチ処理で OCR 可能
...
1. 設定完了後、メイン画面に戻ります
2. PDF ファイルを **ドラッグ＆ドロップ** するか、クリックしてファイルを選択
